...
import { graphqlHandler } from "./apiHandler";
...
describe("Book type", async () => {
  it("Gets an object by GOT ID", async () => {
    ...
    var result = await graphqlHandler(query);
  ...
  it("Gets an object by global ID", async () => {
    ...
    var nextResult = await graphqlHandler(nextQuery);
    expect(result.data.book.name).to.equal("A Game of Thrones");
    ...
    var query = `
{
  ...
  it("Pagination query", async () => {
    ...
    var nextResult = await graphqlHandler(nextQuery);
    expect(
      nextResult.data.allBooks.edges.map((e) => e.node.name)
    ).to.deep.equal(["A Storm of Swords", "The Hedge Knight"]);
  });
});
